...
public ref partial struct Utf8Buffer
{
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public void Append(Utf8String value, int repeat) => Append(value.AsSpan(), repeat);
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public void Append(scoped ReadOnlySpan<byte> value, int repeat)
    {
        var valueLength = value.Length;
    ...
    private void AppendCore(scoped ReadOnlySpan<byte> value, int valueLength, int repeat)
    {
        ...
        scoped var destination = _buffer.Slice(position, appendLength);
